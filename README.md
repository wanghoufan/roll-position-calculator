...
- 结果表中**第 4 阶段（杠杆 15×）**以浅绿底色 + 左侧绿条着重强调，便于快速定位关键档位；强调行由常量 `FOCUS_STAGE` 控制，改一个数字即可换强调行。
- 本金与初始杠杆会记住上次计算的值：每次计算后写入 localStorage，下次打开自动带入；输入区下方「恢复出厂默认（50 U / 20 倍）」按钮可一键还原。
...
- 记录同样存本地，删除需二次确认；
- 任意一条记录都可**点击「编辑」重新载入到上方表单**：修改文字、增删图片，保存后**原地更新**（不会新增重复条目），提交按钮会临时变为「保存修改」。
...
- 编辑已有记录时，也能在该区域**继续新增或删除图片**，保存后生效；
...
| `rollcalc.defaults.v1`（localStorage） | 默认本金与初始杠杆 | 跟随浏览器 |
